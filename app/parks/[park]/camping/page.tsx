...
const PARK_CAMPS: Record<string, any[]> = {
...
export default function ParkCampingPage() {
  ...
  const camps   = PARK_CAMPS[slug] ?? [];
  const [availability, setAvailability] = useState<Record<string, any>>({});
  const [loading,      setLoading]      = useState(false);
  const [selected,     setSelected]     = useState<any>(null);

  useEffect(() => {
    const recgov = camps.filter(c => c.facilityId && !c.note?.includes('Permit'));
  ...
  // Convert to Camp format for CampsiteMap
  const campsForMap = camps.map(c => {
    const live  = availability[c.id];
    return {
      ...c,
  ...
  return (
    <div className="sw" style={{ paddingBottom:'4rem' }}>
      ...
      <p className="ssub">{camps.length} campgrounds · {loading ? 'Checking live availability…' : 'Click a campground for details.'}</p>
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:'1rem' }}>
        {camps.map(c => {
          ...
          const statusColor = avail === 0 ? '#E05050' : avail <= 5 ? '#E0B85C' : '#4ABC78';
          ...
          return (
            <div key={c.id} onClick={() => setSelected(isSel ? null : c)}
              ...
              <a href={c.url} target="_blank" rel="noopener" onClick={e => e.stopPropagation()}
                ...
                {c.res || c.system?.includes('Permit') ? 'Book / Permit →' : 'More Info →'}
              </a>
            </div>
          );
        })}
      ...
      <div style={{ marginTop:'2rem', background:'rgba(74,173,188,.06)', border:'1px solid rgba(74,173,188,.15)',
        ...
        <div style={{ fontWeight:700, fontSize:'.86rem', marginBottom:'.5rem', color:'var(--glacial)' }}>
          💡 Booking tips for {park.shortName}
        </div>
        <div style={{ fontSize:'.8rem', color:'rgba(242,245,247,.7)', lineHeight:1.75 }}>
          Reservations at Recreation.gov open 6 months in advance at 7am Eastern. Set a calendar reminder
          for exactly 6 months before your target arrival date. First-come sites fill by 9–10am on summer weekends — arrive the evening before.
        </div>
      </div>
