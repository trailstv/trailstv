      ...
      <div className="price-grid">
        {TIERS.map(tier => (
          <div key={tier.id} className={`pcrd${tier.highlight ? ' featured' : ''}`}>
            ...
            {tier.id === 'free' ? (
              <Link href="/" className={tier.highlight ? 'bp' : 'bs'} style={{ textAlign:'center' }}>
                {tier.cta}
              </Link>
            ) : (
              <button
                className={tier.highlight ? 'bp' : 'bs'}
                style={{ width:'100%', cursor:'pointer' }}
                onClick={() => alert(`Stripe checkout for ${tier.name} — set up STRIPE_PRICE_${tier.id.toUpperCase()} in Vercel env vars`)}
              >
                {tier.cta}
              </button>
            )}
