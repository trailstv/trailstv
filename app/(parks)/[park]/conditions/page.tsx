...
export default function ConditionsPage() {
  ...
  useEffect(() => {
    ...
    Promise.allSettled([
      ...
      fetch(`/api/nps?parkCode=${park.npsCode}&type=alerts`).then(r => r.json()),
    ]).then(([wr, ar]) => {
  ...
  return (
    <div className="sw" style={{ maxWidth:820, paddingBottom:'4rem' }}>
      ...
      <div className="eye">Live Conditions · {park.shortName}</div>
      <h1 className="stitle">Current Conditions</h1>
      <p className="ssub">Live weather forecast and official NPS alerts for {park.shortName}.</p>

      {loading && <div style={{ color:'var(--granite)', fontSize:'.84rem', marginBottom:'1.5rem' }}>Loading live data…</div>}
        ...
        <section style={{ marginBottom:'2rem' }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:'.65rem' }}>
            {alerts.map((a:any) => {
              ...
              return (
                <div key={a.id} style={{ background:'rgba(13,27,42,.65)',
                  ...
                  <div style={{ display:'flex', justifyContent:'space-between', gap:'1rem', marginBottom:'.3rem' }}>
                    ...
                    <span style={{ background:`${color}20`, color, borderRadius:4,
                      padding:'1px 7px', fontSize:'.68rem', fontWeight:700, flexShrink:0 }}>
                      {a.category}
                    </span>
                  </div>
                  ...
                  {a.url && <a href={a.url} target="_blank" rel="noopener"
                    style={{ fontSize:'.72rem', color:'var(--glacial)', fontWeight:600, display:'inline-block', marginTop:'.5rem' }}>
                    Full details →
                  </a>}
                </div>
              );
        ...
        <section style={{ marginBottom:'2rem' }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(140px,1fr))', gap:'.65rem', marginBottom:'1.25rem' }}>
            {([
              ...
              <div key={label} style={{ background:'rgba(13,27,42,.6)', border:'1px solid var(--cborder)',
                borderRadius:9, padding:'.7rem .85rem' }}>
                <div style={{ fontSize:'.64rem', color:'var(--granite)', textTransform:'uppercase',
                  letterSpacing:'.08em', marginBottom:'.2rem' }}>{label}</div>
      ...
      <div style={{ marginTop:'1.5rem', display:'flex', gap:'.75rem', flexWrap:'wrap' }}>
        <a href={`https://www.nps.gov/${park.npsCode}/planyourvisit/conditions.htm`}
          target="_blank" rel="noopener" className="bs">Full NPS Conditions →</a>
        <Link href={`/parks/${slug}/fire`} className="bs">Fire Restrictions →</Link>
        <Link href={`/parks/${slug}/snow`} className="bs">Snow Report →</Link>
      </div>
    </div>
  );
}
